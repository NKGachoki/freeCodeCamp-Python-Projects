def arithmetic_arranger(problems,answer=False):
    """Function to arrange and solve list of problems"""
    ...
    if answer is True:
        ...
        # Formatting lists to remove brackets, commas and quotation marks
        format_top = (' ' * 4).join(x for x in top)
        format_bottom = (' ' * 4).join(x for x in bottom)
        format_dashes = (' ' * 4).join(x for x in dashes)
        format_answers = (' ' * 4).join(x for x in answers)

        # Storing formatted lists in variable
        arranged_problems = f"{format_top}\n{format_bottom}\n{format_dashes}\n{format_answers}"
        return arranged_problems
